      ...
      <div className="col-9 align-self-center">
        <div>
          ...
          <span className="d-flex justify-content-between">
            <Button className="border-0 bg-white" onClick={handleShowModal}>
              <span className="body-font border border-1 border-warning rounded text-warning fw-bold">
                View Details
              </span>
            </Button>
            ...
            <Modal show={showModal} onHide={handleCloseModal}>
              <Modal.Header>
                <Modal.Title>Order Number - #{props.id}</Modal.Title>
              </Modal.Header>
              <Modal.Body className="m-3">
                <div className="row">
                  <div className="body-font text-end text-muted">
                    {formattedDate}
                  </div>
                </div>

                <div className="fw-bold row  text-muted sub-heading ">
                  Items
                </div>
                {props.items.map((item) => {
                  return (
                    <div className=" row m-2 sub-heading" key={item.item_id}>
                ...
                <div className="star-rating heading ">
                  <span className="fw-bold  text-danger body-font mx-4 ">
                    {rating ? "Rated" : "Rate Us"}
                  </span>
                ...
                {rating && !props.overall_rating && (
                  <div className="mb-3 heading border shadow border-primary ">
                    <textarea
                      rows="3"
                ...
                {props.orderreview && <span className="body-font text-body"><b className="body-font">Comment:</b> {props.orderreview}</span>}
              </Modal.Body>
              <Modal.Footer>
        ...
        <div className="body-font">
          {props.items.map((item) => {
            return (
              <span key={item.item_id} className="body-font text-muted">
                {item.item_name} x {item.quantity}
                {"  "}
              </span>
            );
